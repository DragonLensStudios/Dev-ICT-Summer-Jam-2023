...
namespace DLS.Dialogue
{
    ...
    public class VariablesObject : ScriptableObject, ISerializationCallbackReceiver
    {
        ...
        private void SetValue<T>(List<IVariable<T>> variableList, string variableName, T value) where T : IComparable<T>, IEquatable<T>
        {
        ...
        private T GetValue<T>(List<IVariable<T>> variableList, string variableName) where T : IComparable<T>, IEquatable<T>
        {
            IVariable<T> variable = variableList.Find(v => v.Name == variableName);
            if (variable != null)
            {
