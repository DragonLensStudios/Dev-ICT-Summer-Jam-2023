...
public class PlayerPrefsOverview : EditorWindow
{
    ...
    void OnEnable()
    {
        sortType = (SortType)Enum.Parse(typeof(SortType),
            PlayerPrefs.GetString("PlayerPrefsOverview_SortType", "NameAZ"));
        searchType = (SearchType)Enum.Parse(typeof(SearchType),
            PlayerPrefs.GetString("PlayerPrefsOverview_SearchType", "Name"));
    ...
    {
        "UnityGraphicsQuality", "PlayerPrefsOverview_SortType", "PlayerPrefsOverview_SearchWord",
        "PlayerPrefsOverview_SearchType"
    };
    ...
    void OnGUI()
    {
        ...
        EditorGUILayout.BeginHorizontal(EditorStyles.toolbar);
        {
            ...
            if (GUILayout.Button(new GUIContent("Sort"), EditorStyles.toolbarButton, GUILayout.Width(35)))
            {
                ...
                _sort.AddItem(new GUIContent("Name: A-Z", ""), (sortType == SortType.NameAZ) ? true : false,
                    SortKeyList, SortType.NameAZ);
                _sort.AddItem(new GUIContent("Name: Z-A", ""), (sortType == SortType.NameZA) ? true : false,
                    SortKeyList, SortType.NameZA);
                _sort.AddItem(new GUIContent("Value: A-Z", ""), (sortType == SortType.ValueAZ) ? true : false,
                    SortKeyList, SortType.ValueAZ);
                _sort.AddItem(new GUIContent("Value: Z-A", ""), (sortType == SortType.ValueZA) ? true : false,
                    SortKeyList, SortType.ValueZA);
                _sort.AddItem(new GUIContent("Type: A-Z", ""), (sortType == SortType.TypeAZ) ? true : false,
                    SortKeyList, SortType.TypeAZ);
                _sort.AddItem(new GUIContent("Type: Z-A", ""), (sortType == SortType.TypeZA) ? true : false,
                    SortKeyList, SortType.TypeZA);
                _sort.DropDown(sortRect);
            }

            GUILayout.Space(5);
            if (searchWord != (searchWord = EditorGUILayout.TextArea(searchWord, EditorStyles.toolbarTextField,
            ...
            if (GUILayout.Button("▼  .", EditorStyles.toolbarButton, GUILayout.Width(25)))
            {
                ...
                _search.AddItem(new GUIContent("Name", ""), (searchType == SearchType.Name) ? true : false,
                    SearchTypeList, SearchType.Name);
                _search.AddItem(new GUIContent("Value", ""), (searchType == SearchType.Value) ? true : false,
                    SearchTypeList, SearchType.Value);
            ...
            {
                if (autoRefresh != (autoRefresh = GUILayout.Toggle(autoRefresh,
                        new GUIContent("Auto", "Automatically refreshes"), EditorStyles.toolbarButton,
                        GUILayout.Width(35))))
                {
                    GUI.FocusControl("");
                }
            ...
            if (GUILayout.Button(new GUIContent("Refresh", "Force a refresh"), EditorStyles.toolbarButton,
                    GUILayout.Width(50)))
            {
                GUI.FocusControl("");
                UpdateKeyList();
        ...
        EditorGUILayout.BeginHorizontal();
        {
            ...
            else
            {
                if (selectedAll != (selectedAll =
                        GUILayout.Toggle(selectedAll, new GUIContent("", "Toggle all"), GUILayout.Width(15))))
                {
                    SelectedAll(selectedAll);
                }
            }
        ...
        if (showNew)
        {
            EditorGUILayout.BeginHorizontal();
            {
                ...
                if (newName != (newName = EditorGUILayout.TextArea(newName, keyInput, GUILayout.MinWidth(100))))
                {
                    newNameError = false;
                }
                ...
                {
                    newValueString = EditorGUILayout.TextArea(newValueString, keyInput, GUILayout.MinWidth(100));
                }
                else if (newKeyTypeIndex == 1)
                {
                    newValueFloat = EditorGUILayout.FloatField(newValueFloat, keyInput, GUILayout.MinWidth(100));
                }
                else if (newKeyTypeIndex == 2)
                {
                    newValueInt = EditorGUILayout.IntField(newValueInt, keyInput, GUILayout.MinWidth(100));
                }
        ...
        if (searchKeys.Count != 0)
        {
            scrollPos = EditorGUILayout.BeginScrollView(scrollPos);
            {
                for (int i = 0; i < searchKeys.Count; i++)
                {
                    ...
                    if (searchKeys[i].selected != (searchKeys[i].selected = GUILayout.Toggle(searchKeys[i].selected,
                            new GUIContent("", "Selected"), GUILayout.Width(15))))
                    {
                        Selected();
                    }
                    ...
                    if (searchKeys[i].name != (searchKeys[i].name =
                            EditorGUILayout.TextArea(searchKeys[i].name, keyInput, GUILayout.MinWidth(70))))
                    {
                        searchKeys[i].Edit();
                    }
                    ...
                    {
                        if (searchKeys[i].value != (searchKeys[i].value =
                                EditorGUILayout.TextArea((string)searchKeys[i].value, keyInput,
                    ...
                    {
                        if (searchKeys[i].value != (searchKeys[i].value =
                                EditorGUILayout.IntField((int)searchKeys[i].value, keyInput, GUILayout.MinWidth(70))))
                        {
                            searchKeys[i].Edit();
                        }
                    }
                    ...
                    {
                        if (searchKeys[i].value != (searchKeys[i].value =
                                EditorGUILayout.FloatField((float)searchKeys[i].value, keyInput,
                                    GUILayout.MinWidth(70))))
                        {
                            searchKeys[i].Edit();
                        }
    ...
    private void Search()
    {
        ...
        else
        {
            ...
            {
                searchKeys = keys.Where(o => keys.Any(folder => o.name.ToLower().Contains(searchWord.ToLower())))
                    .ToList();
            }
            else if (searchType == SearchType.Value)
            {
                searchKeys = keys.Where(o =>
                    keys.Any(folder => o.value.ToString().ToLower().Contains(searchWord.ToLower()))).ToList();
            }
        }
    ...
    private void Export(object _all)
    {
        ...
        string _path =
            EditorUtility.SaveFilePanelInProject("Export PlayPrefs Keys", "PlayerPrefs", "ppo",
                "Export PlayPrefs Keys");
    ...
    string[] GetAllKeys()
    {
        if (Application.platform == RuntimePlatform.WindowsEditor)
        {
            ...
            RegistryKey _registryKey = _currentUser.CreateSubKey("Software\\Unity\\UnityEditor\\" +
